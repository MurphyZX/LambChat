...
class TraceStorage(
    ...
    async def get_first_trace_events_batch(
        ...
        """
        bounded_ids = _bounded_unique_strings(trace_ids, TRACE_LIST_LIMIT)
        if not bounded_ids:
            return {}
        bounded_types = _bounded_unique_strings(event_types, SESSION_EVENT_FILTER_LIST_LIMIT)
        ...
        for trace_id in bounded_ids:
            ...
            for index, event in enumerate(legacy_events_by_trace.get(trace_id) or [], start=1):
                if (
                    ...
                    and _event_seq(event, index) >= first_chunk_start_seq
                ):
                    continue
                if _accepts(event):
                    first = event
    ...
    async def list_run_summaries(
        ...
        try:
            ...
            # 缺 first_user_message_preview 的旧 trace 批量回填（1 次 chunks
            # 聚合 + 1 次 legacy 查询，替代逐条 get_first_trace_event 的 N+1）
            missing_trace_ids = [
                str(trace.get("trace_id"))
                for trace in traces
                if trace.get("trace_id") and not (trace.get("first_user_message_preview") or {})
            ]
            first_events_by_trace = (
                await self.get_first_trace_events_batch(
                    missing_trace_ids,
            ...
            for trace in traces:
                ...
                if not preview and trace.get("trace_id"):
                    preview = first_events_by_trace.get(str(trace.get("trace_id"))) or {}
                if preview:
                ...
                next_seq = (
                    max(
                        (
                            ...
                            if event.get("seq") is not None and str(event.get("seq")).isdigit()
                        ),
                        default=0,
                    )
                    + 1
